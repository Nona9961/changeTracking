 ...
 * <p>
 * 叶子节点：字段两侧的节点类型不同且至少一侧是容器节点（ObjectNode/CollectionNode）
 * 时（如 ObjectNode↔NullNode、ObjectNode↔PrimitiveNode、CollectionNode↔ObjectNode、
 * CollectionNode↔NullNode 等），快照中<b>没有业务对象可提取</b>，只有 ValueNode 表示——
 * 本节点原样携带两侧节点，由消费方按类型解读（NullNode=清空、ObjectNode=赋值、
 * PrimitiveNode=值），无需下钻子节点。
 * <p>
